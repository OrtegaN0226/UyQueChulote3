...
(function () {
    ...
    Game_Map.prototype.scrollLeft = function (distance) {
        ...
        if (typeof this.scrollX_OC === "function") this.scrollX_OC(distance, lastX - this._displayX);
    };
    ...
    Game_Map.prototype.scrollUp = function (distance) {
        ...
        if (typeof this.scrollY_OC === "function") this.scrollY_OC(distance, lastY - this._displayY);
    };
    ...
    Game_Map.prototype.setDisplayPos = function (x, y) {
        ...
        // Sync OcRam fixed-to-map and parallax layers to the new display position
        if (window.OcRam && OcRam.Layers && typeof OcRam.twh !== "undefined") {
            try {
                const px = this._displayX * OcRam.twh[0];
                const py = this._displayY * OcRam.twh[1];
                const pls = OcRam.Layers.getParallaxLayers ? OcRam.Layers.getParallaxLayers() : [];
                const dls = OcRam.Layers.getDynamicLayers ? OcRam.Layers.getDynamicLayers() : [];
    ...
    ScSWinter.Scene_Map_onMapLoaded = Scene_Map.prototype.onMapLoaded;
    Scene_Map.prototype.onMapLoaded = function () {
        // Allow other plugins (e.g., OcRam) to run their onMapLoaded logic first
        ScSWinter.Scene_Map_onMapLoaded.call(this);

        var s_zoom = parseFloat(s_extractNoteValue($dataMap.note, "Zoom:")) || 0.0;
        if (s_zoom < 0.5) s_zoom = ScSWinter.DefaultZoom;
        $gameMap.extraScreenTile(s_zoom > 1.0 ? s_zoom : 1);
        if (s_zoom > 1.0) {
