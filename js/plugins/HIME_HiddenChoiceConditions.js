...
@author Hime --> HimeWorks (http://himeworks.com) (UPDATED FOR RPGMMZ BY JACKHOFFX)
@version 1.2
@date Jan 5, 2016
...
@target MZ
@plugindesc Allows you to hide choices with a simple event call (MV/MZ). 

@command hide_choice
@text Hide Choice
@desc Hide a specific choice (1-based index) for the next Show Choices.
@arg choiceNumber
@text Choice Number
@type number
@min 1
...
(function ($) {
  ...
  Game_Message.prototype.backupChoices = function() {
    console.log('[HIME_HiddenChoice] backupChoices called. choices len=', (this._choices || []).length);
    this._oldChoices = (this._choices || []).slice();
  }
  
  Game_Message.prototype.restoreChoices = function() {
    console.log('[HIME_HiddenChoice] restoreChoices called. oldChoices len=', (this._oldChoices || []).length);
  ...
  Game_Message.prototype.clear = function() {
    ...
    this._oldChoices = [];
    console.log('[HIME_HiddenChoice] Game_Message.clear: reset hidden conditions.');
  };
  
  /* Returns whether the specified choice is hidden */
  Game_Message.prototype.isChoiceHidden = function(choiceNum) {
    const hidden = this._hiddenChoiceConditions[choiceNum];
    // console.log('[HIME_HiddenChoice] isChoiceHidden?', choiceNum, '=>', hidden);
    return hidden;
  }
  
  Game_Message.prototype.hideChoice = function(choiceNum, bool) {
    console.log('[HIME_HiddenChoice] Game_Message.hideChoice:', 'index=', choiceNum, 'hidden=', !!bool);
  ...
  Game_Interpreter.prototype.setupChoices = function(params) {
    console.log('[HIME_HiddenChoice] Interpreter.setupChoices: backing up choices and continuing');
    TH_HiddenChoices_GameInterpreter_setupChoices.call(this, params);
    $gameMessage.backupChoices();
  };
  
  /* Use a plugin command - MV compatibility */
  if (Game_Interpreter.prototype.pluginCommand) {
    ...
    Game_Interpreter.prototype.pluginCommand = function(command, args) {
      ...
      if (command && command.toLowerCase() === "hide_choice") {
        console.log('[HIME_HiddenChoice] MV pluginCommand hide_choice:', args);
        var choiceNum = Math.floor((Number(args[0]) || 0) - 1);
  ...
  if (PluginManager && PluginManager.registerCommand) {
    PluginManager.registerCommand("HIME_HiddenChoiceConditions", "hide_choice", function(args) {
      console.log('[HIME_HiddenChoice] MZ registerCommand hide_choice:', args);
      var choiceNum = Math.floor((Number(args.choiceNumber) || 0) - 1);
      $gameMessage.hideChoice(choiceNum, true);
    });
  }
  
  Game_Interpreter.prototype.hideChoice = function(choiceNum, formula) {
    console.log('[HIME_HiddenChoice] Interpreter.hideChoice called:', 'choiceNum=', choiceNum, 'formula=', formula);
  ...
  hide_choice = function(choiceNum, formula) {
    console.log('[HIME_HiddenChoice] global hide_choice called:', 'choiceNum=', choiceNum, 'formula=', formula);
    var num = Math.floor(choiceNum) - 1;    
    $gameMessage.hideChoice(num, eval(formula));
  };
  ...
  Window_ChoiceList.prototype.makeCommandList = function() {
    console.log('[HIME_HiddenChoice] Window_ChoiceList.makeCommandList: start');
    ...
    for (var i = this._list.length; i > -1; i--) {
      if ($gameMessage.isChoiceHidden(i)) {
        console.log('[HIME_HiddenChoice] Removing hidden choice at index', i);
    ...
    if (needsUpdate === true) {
       console.log('[HIME_HiddenChoice] Hidden choices removed; updating placement');
