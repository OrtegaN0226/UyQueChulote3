 ...
 * @default 1.0
 *
 * @param Default zoom mode
 * @desc Choose how default zoom is resolved.
 * @type select
 * @option Static
 * @value static
 * @option From platform (JackHoffx_Platform)
 * @value platform
 * @default static
 *
 * @param Default zoom (PC)
 * @desc Default zoom when edition is PC.
 * @type number
 * @decimals 2
 * @default 1.5
 *
 * @param Default zoom (Mobile)
 * @desc Default zoom when edition is Mobile.
 * @type number
 * @decimals 2
 * @default 1.8
 *
 ...
 *
 * @param Default zoom mode
 * @desc Escolha como o zoom padrão é definido.
 * @type select
 * @option Static
 * @value static
 * @option From platform (JackHoffx_Platform)
 ...
 *
 * @param Default zoom (PC)
 * @desc Zoom padrão quando a edição é PC.
 * @type number
 * @decimals 2
 * @default 1.5
 *
 * @param Default zoom (Mobile)
 * @desc Zoom padrão quando a edição é Mobile.
 * @type number
 * @decimals 2
 * @default 1.8
 *
...
(function ($) {
  ...
  const pluginName = "MBS_MapZoom";
  $.Parameters = ($plugins.filter(function(p) {return p.description && p.description.indexOf('<MBS MapZoom>') >= 0;})[0] || {}).parameters || {};
  $.Param = $.Param || {};
  ...
  // Default zoom ratio
  $.Param.defaultZoom = Number($.Parameters["Default zoom"] || 1.0);
  $.Param.defaultZoomMode = String($.Parameters["Default zoom mode"] || "static").toLowerCase();
  $.Param.defaultZoomPc = Number($.Parameters["Default zoom (PC)"] || 1.5);
  $.Param.defaultZoomMobile = Number($.Parameters["Default zoom (Mobile)"] || 1.8);

  function resolveDefaultZoom() {
    if ($.Param.defaultZoomMode === "platform") {
      const hasFn = typeof window !== "undefined" && typeof window.isMobileEdition === "function";
      const isMobile = hasFn
        ? !!window.isMobileEdition()
        : !!(window && window.GAME_EDITION && window.GAME_EDITION.IS_MOBILE);
      return isMobile ? $.Param.defaultZoomMobile : $.Param.defaultZoomPc;
    }
    return $.Param.defaultZoom;
  }
  ...
  Game_Map.prototype.initialize = function() {
    ...
    this._zoom = this._zoom || new PIXI.Point(defaultZoom, defaultZoom);
  ...
  Game_Map.prototype.setup = function(mapId) {
    ...
    if ($.Param.resetOnMapChange)
    	this._zoom = new PIXI.Point(resolveDefaultZoom(), resolveDefaultZoom());
  };
  ...
  Game_Map.prototype.setZoom = function(x, y, duration) {
    // Lazy-init zoom state in case initialize hasn't run yet
    if (!this._destZoom) this._destZoom = new PIXI.Point(1,1);
    if (!this._zoom) {
      const defaultZoom = resolveDefaultZoom();
      this._zoom = new PIXI.Point(defaultZoom, defaultZoom);
    }
    if (!this._spdZoom) this._spdZoom = new PIXI.Point(0,0);
  ...
  // On MV we need to adjust center to account for zoom; on MZ the stock math plus our screenTileX/Y override is sufficient.
  if (Utils && Utils.RPGMAKER_NAME === "MV") {
  ...
  // Preserve native MZ Tilemap layers; use custom layers only on MV.
  const _Tilemap_createLayers_MZ = Tilemap.prototype._createLayers;
  Tilemap.prototype._createLayers = function() {
    if (Utils && Utils.RPGMAKER_NAME === "MZ") {
      return _Tilemap_createLayers_MZ.call(this);
    }
  ...
  Game_Interpreter.prototype.pluginCommand = function (command, args) {
  	...
  	if (command === "MapZoom") handleMapZoomCommand(args);
  };
  ...
  if (PluginManager.registerCommand) {
    PluginManager.registerCommand(pluginName, "set", args => {
      // args: x, y, duration
      const baseZoom = resolveDefaultZoom();
      const x = Number(args.x || args.ratio || 1) * baseZoom;
      const y = args.y ? Number(args.y) * baseZoom : x;
      const duration = args.duration ? Number(args.duration) : 60;
      $gameMap.setZoom(x, y, duration);
    });
    PluginManager.registerCommand(pluginName, "reset", args => {
      const duration = args.duration ? Number(args.duration) : 60;
      const baseZoom = resolveDefaultZoom();
      $gameMap.setZoom(baseZoom, baseZoom, duration);
      $gameMap.setZoomCenter();
    });
    PluginManager.registerCommand(pluginName, "center", args => {
      // mode: reset | event | point
      if (args.mode === "reset") {
        $gameMap.setZoomCenter();
      } else if (args.mode === "event") {
        const ev = $gameMap.event(Number(args.id || 0));
        if (ev) $gameMap.setZoomCenter(ev);
      } else {
        $gameMap.setZoomCenter(Number(args.x || 0), Number(args.y || 0));
      }
    });
  }

  function handleMapZoomCommand(args){
    ...
    if(cmd==="set"){
      ...
      if(args[1]){
        if(args[2]){
          if(args[2].toLowerCase()==="duration" && args[3]){
            $gameMap.setZoom(Number(args[1]) * baseZoom, Number(args[1]) * baseZoom, Number(args[3]));
          }else if(args[3] && args[3].toLowerCase()==="duration" && args[4]){
            $gameMap.setZoom(Number(args[1]) * baseZoom, Number(args[2]) * baseZoom, Number(args[4]));
          }else{
            $gameMap.setZoom(Number(args[1]) * baseZoom, Number(args[2]) * baseZoom);
          }
        }else{
      ...
      const baseZoom = resolveDefaultZoom();
      if(args[1]){
        $gameMap.setZoom(baseZoom, baseZoom, Number(args[1]));
      }else{
        $gameMap.setZoom(baseZoom);
      }
      $gameMap.setZoomCenter();
    }else if(cmd==="center"){
      if(args[1]==="event"){
        const ev=$gameMap.event(Number(args[2]));
        if(ev) $gameMap.setZoomCenter(ev);
      ...
      $gameMap.setZoom($gameMap._destZoom.x, $gameMap._destZoom.y, $gameMap._zoomDuration - ($gameMap._spdZoom || 0));
    }
  }
